...
public class Kangaroo implements Listener {
	...
	public static HashMap<String, Integer> KangarooStats = new HashMap<>();
		...
		if ((KitAPI.getKit(p) == "Kangaroo") && (e.getMaterial() == Material.FIREWORK)) {
			...
			if (p.isSneaking()) {
				if (!F.contains(p.getName())) {
					...
					KangarooStats.put(p.getName(), Integer.valueOf(1));
				} else {
					if (KangarooStats.get(p.getName()) != 2) {
						p.setVelocity(p.getLocation().getDirection().multiply(1.2D));
						p.setVelocity(new Vector(p.getVelocity().getX(), 0.5D, p.getVelocity().getZ()));
						KangarooStats.put(p.getName(), Integer.valueOf(2));
		...
		if (KitAPI.getKit(p) == "Kangaroo") {
			if (b.getRelative(BlockFace.DOWN).getType() == Material.AIR) {
				...
				if (b.getType() != Material.AIR) {
					KangarooStats.put(p.getName(), Integer.valueOf(0));
					F.remove(p.getNoDamageTicks());
				}
